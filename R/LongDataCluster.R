...
#' @param No.Class Optional, the number of clusters if user has a priori in mind. Default is \code{NULL}.
...
LongDataCluster <- function(x, Y, id, 
  ...
  gap.ID = res$out.ID[[N.cluster]]
  CH.ID  = res$out.ID[[which.max(CH.index)]]
  label.gap <- cbind(id = unlist(gap.ID), 
                     label.Gapb = rep(seq(length(gap.ID)), unlist(lapply(gap.ID, length))))
  label.CH <- cbind(id = unlist(CH.ID), 
                    label.CH = rep(seq(length(CH.ID)), unlist(lapply(CH.ID, length))))
  Dat.label = cbind(id, x, data.matrix(Y))
  Dat.label = merge(Dat.label, label.CH, by = "id")
  Dat.label = merge(Dat.label, label.gap, by = "id")
